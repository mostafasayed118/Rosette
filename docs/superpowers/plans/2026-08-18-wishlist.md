...
vi.mock('@/lib/supabase/browser', () => ({ getBrowserSupabase: () => null }));
// `useStorePath` reads `useParams` (returns null without a router context).
...
Run: `npx vitest run tests/domain/wishlist-cron.test.ts`
Expected: PASS (7/7: 2 render + 5 cron — price-drop, back-in-stock, no-change, send-failure, missing-email).
