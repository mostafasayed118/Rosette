...
describe('GiftFinderQuiz', () => {
  it('walks through the questions and shows results on completion', async () => {
    ...
    fireEvent.click(await screen.findByText('A partner'));
    fireEvent.click(await screen.findByText('Celebration')); // EN value of the `celebration` key (the birthday occasion)
    fireEvent.click(await screen.findByText('EGP 150–250'));
    fireEvent.click(await screen.findByText('Red'));
    fireEvent.click(await screen.findByText('Romantic'));
