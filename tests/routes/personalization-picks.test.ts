...
describe('GET /api/personalization/picks', () => {
  ...
  it('clamps limit via zod default and handles locale ar', async () => {
    ...
    mockGetProvider.mockReturnValue({ getPicks: getPicksSpy } as any);
  ...
  it('falls back on provider error and logs', async () => {
    mockCreateClient.mockReturnValue({
      auth: { getUser: vi.fn(async () => ({ data: { user: { id: 'uid' } } })) },
    } as any);
    ...
    const fallbackProvider = {
      getPicks: vi.fn(async () => makePicks({ buyAgain: [], recommended: [], reason: 'fallback' })),
    };
    // first call fails, second succeeds - route's catch will call getPersonalizationProvider again
    mockGetProvider.mockImplementation(() => failingProvider as any);
    // make second invocation return fallback (simulate second call)
    let call = 0;
    mockGetProvider.mockImplementation(() => {
      call++;
      return (call === 1 ? failingProvider : fallbackProvider) as any;
    });
    ...
    const body = await res.json();
    expect(body.reason).toBe('fallback');
  });
