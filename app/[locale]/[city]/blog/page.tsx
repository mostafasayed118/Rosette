...
export default async function BlogPage({ params }: BlogPageParams) {
  ...
  return <div className="flex min-h-screen flex-col"><SiteHeader /><main className="mx-auto w-[min(calc(100%-3rem),80rem)] py-12 pb-24 max-md:w-[min(calc(100%-2rem),80rem)] max-md:pt-4"><LocalizedPageHeading eyebrow="blogEyebrow" title="blogTitle" lede="blogLede" /><div className="mt-10 grid gap-6 md:grid-cols-2 lg:grid-cols-3"><Link className="group flex flex-col rounded-2xl border border-border p-6 transition-colors hover:border-primary" href={`/${locale}/${citySlug}/delivery`}><span className="text-xs font-bold uppercase tracking-[.16em] text-sage">{t('blogDeliveryCity')}</span><span className="mt-2 font-display text-2xl leading-tight text-foreground group-hover:text-primary">{t('blogDeliveryTitle', { city: cityName ?? citySlug })}</span><span className="mt-2 text-sm text-muted-foreground">{t('blogDeliveryLede')}</span></Link>{posts.map((post) => (<Link className="group flex flex-col rounded-2xl border border-border p-6 transition-colors hover:border-primary" href={`/${locale}/${citySlug}/blog/${post.slug}`} key={post.id}><span className="text-xs font-bold uppercase tracking-[.16em] text-sage">{post.category ? t(`blogCategory_${post.category}`) : t('blogCategory_guides')}</span><span className="mt-2 font-display text-2xl leading-tight text-foreground group-hover:text-primary">{post.titleAr ? (locale === 'ar' ? post.titleAr : post.titleEn) : post.titleEn}</span><span className="mt-2 line-clamp-3 text-sm text-muted-foreground">{post.excerptAr ? (locale === 'ar' ? post.excerptAr : post.excerptEn) : post.excerptEn}</span></Link>))}</div></main><SiteFooter locale={locale} city={citySlug} /></div>;
}
