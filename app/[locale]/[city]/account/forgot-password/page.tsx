...
export default function ForgotPasswordPage() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    if (!supabase) return;
    setSubmitting(true);
    await supabase.auth.resetPasswordForEmail(email.trim(), { redirectTo: `${window.location.origin}${href('/account/reset-password')}` });
    setSent(true);
    setSubmitting(false);
  }
    ...
    <main id="main-content" className="mx-auto w-[min(calc(100%-3rem),80rem)] py-12 pb-24 max-md:w-[min(calc(100%-2rem),80rem)] max-md:pt-4">
      <section className="mx-auto grid max-w-[34rem] gap-5 rounded-2xl border bg-card p-8 shadow-sm">
        ...
        <p className="text-xs text-muted-foreground">
          <Link className="text-primary underline underline-offset-4" href={href('/account/login')}>{t('signIn')}</Link>
        </p>
      </section>
    </main>
  );
