...
export default async function AdminOrderDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <main className="content-frame">
    ...
    <section className="form-section"><p className="eyebrow">{t('recipientAndDelivery')}</p>
      ...
      {whatsapp ? <p><a className="button" href={whatsapp} target="_blank" rel="noopener noreferrer">{t('contactOnWhatsApp')}</a></p> : null}
    </section>

    <section className="form-section"><p className="eyebrow">{t('items')}</p>
      {((order.order_items ?? []) as Array<{ id: string; product_name_en: string; unit_price_minor: number; quantity: number }>).map((item) => (
        <p key={item.id}>{item.product_name_en} × {item.quantity} · {formatMoney(item.unit_price_minor, locale)}</p>
      ))}
    </section>

    <section className="form-section"><p className="eyebrow">{t('payment')}</p>
      {((order.payments ?? []) as Array<{ id: string; provider: string; provider_reference: string | null; amount_minor: number; status: string }>).map((payment) => (
        <p key={payment.id}>{payment.provider} · {payment.provider_reference ?? 'n/a'} · {formatMoney(payment.amount_minor, locale)} · {payment.status}</p>
      ))}
    </section>

    <section className="form-section"><p className="eyebrow">{t('timeline')}</p>
