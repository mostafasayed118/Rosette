...
export default async function AdminProductsPage() {
  ...
  return <main className="content-frame"><p className="eyebrow">Catalog operations</p><h1>Products</h1><p><Link className="button" href="/admin/products/new">New product</Link></p><div className="admin-table">{(data ?? []).map((product) => <article className="status-message" key={product.id}><Link href={`/admin/products/${product.id}`}><strong>{product.name_en}</strong></Link><span>{product.name_ar} · {product.price_minor / 100} EGP · {product.active ? 'Active' : 'Inactive'}</span></article>)}</div></main>;
}
