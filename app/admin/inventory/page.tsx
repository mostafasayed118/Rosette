...
export default async function AdminInventoryPage() {
  ...
  return <main className="content-frame"><p className="eyebrow">Stock operations</p><h1>Inventory</h1><div className="admin-table">{(data ?? []).map((row) => <article className="status-message" key={row.variant_id}><strong>{row.variant_id}</strong><span>{Math.max(0, row.quantity - row.reserved_quantity)} available · {row.reserved_quantity} reserved</span><SetQuantityForm variantId={row.variant_id} current={row.quantity} /></article>)}</div></main>;
}
