  ...
  return (
    <div className="grid gap-6">
      ...
      <div className="flex justify-end">
        <Button asChild><Link href="/admin/subscriptions/plans/new">New plan</Link></Button>
      </div>
        ...
        {((plans ?? []) as any[]).map((plan) => (
          <Card key={String(plan.id)}>
            <CardContent className="flex flex-wrap items-center gap-3">
              ...
              <Badge variant={plan.active ? 'default' : 'outline'}>{plan.active ? 'active' : 'inactive'}</Badge>
              <span className="text-sm text-on-surface-variant">
                {((plan.bundle_prices ?? []) as any[]).map((bp) => `${bp.deliveries}×${formatMoney(Number(bp.priceMinor), locale)}`).join(' · ')}
              </span>
              <span className="ml-auto flex gap-2">
                <Button asChild variant="outline" size="sm"><Link href={`/admin/subscriptions/plans/${String(plan.id)}`}>Edit</Link></Button>
              </span>
            </CardContent>
          </Card>
        ))}
