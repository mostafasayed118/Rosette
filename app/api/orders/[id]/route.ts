...
export async function GET(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const { data: order, error } = await getAdminSupabase().from('orders').select('id,display_number,recipient_name,delivery_address,delivery_date,delivery_window,subtotal_minor,delivery_fee_minor,total_minor,payment_status,fulfillment_status,order_items(id,product_name_en,product_name_ar,product_name_fr,unit_price_minor,quantity,add_ons)').eq('id', id).eq('public_token', token).maybeSingle();
