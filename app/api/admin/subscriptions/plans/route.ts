...
function mapPlan(row: any) {
  return {
    ...
    frequencies: row.frequencies ?? [], bundlePrices: row.bundle_prices ?? [],
    productId: row.product_id ? String(row.product_id) : null, active: row.active !== false, sortOrder: Number(row.sort_order ?? 0),
  };
}
...
export async function GET() {
  ...
  return NextResponse.json({ items: ((data ?? []) as any[]).map(mapPlan) });
}
