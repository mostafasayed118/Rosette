...
export function OrderPageContent({ id, token }: { id: string; token?: string }) {
  ...
  const paymentCopy = order.payment.method === 'demo-card' ? t('demoOrder') : order.payment.status === 'authorized' ? (locale === 'ar' ? 'تم تأكيد الدفع.' : 'Payment confirmed.') : (locale === 'ar' ? 'الدفع قيد الانتظار.' : 'Payment is pending.');
