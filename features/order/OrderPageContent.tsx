...
export function OrderPageContent({ id, token }: { id: string; token?: string }) {
  ...
  if (!order) return <StatusMessage title={t('orderMissing')}>{t('startAgain')} <Link className="text-primary underline underline-offset-4" href={href('/shop')}>{t('shop')}</Link>.</StatusMessage>;
