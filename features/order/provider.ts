...
const localOrderRepository: OrderRepository = {
  async createPending(input) {
    ...
    return { ok: true, value: { id: result.value.id, displayNumber: result.value.displayNumber, totalMinor: result.value.totals.total, subtotalMinor: result.value.totals.subtotal, deliveryFeeMinor: result.value.totals.deliveryFee, discountMinor: 0, giftCardMinor: 0, giftCardId: null, giftCardHoldId: null, giftCardCodeLast4: null, publicToken: undefined, paymentStatus: 'pending', fulfillmentStatus: 'confirmed' } };
  },
