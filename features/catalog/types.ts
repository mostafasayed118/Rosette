...
export type DeliveryTier = 'same_day' | 'next_day';
export type Product = { slug: string; name: string; nameAr?: string; nameFr?: string; description: string; descriptionAr?: string; descriptionFr?: string; category: string; occasions: string[]; giftRecipients?: string[]; giftStyles?: string[]; giftColors?: string[]; price: number; tone: string; imageUrl: string | null; inventory: number; delivery: string; deliveryTier?: DeliveryTier; createdAt: string; variants: ProductVariant[]; addOns: AddOn[]; rating?: { average: number; count: number } };
