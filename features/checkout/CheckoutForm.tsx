...
export function CheckoutForm({ cityCode, availablePaymentMethods = defaultPaymentMethods }: { cityCode: string; availablePaymentMethods?: PaymentMethod[] }) {
  ...
  const [input, setInput] = useState<CheckoutInput>(() => ({ recipientName: '', recipientPhone: '', address: '', senderName: '', senderEmail: '', deliveryDate: defaultDeliveryDate(new Date()), deliveryWindow: '12-3', paymentMethod: availablePaymentMethods[0] ?? 'pay-on-delivery' }));
  ...
  return <form className="grid max-w-[60rem] gap-6 pt-8" onSubmit={submit} noValidate>
    ...
    <section className="grid gap-4 border-b py-6"><p className="text-xs font-bold uppercase tracking-[.16em] text-sage">{t('paymentEyebrow')}</p><div className="grid gap-1.5"><span className="text-sm font-bold text-foreground">{t('paymentMethod')}</span><Select value={input.paymentMethod} onValueChange={(value) => update('paymentMethod', value as CheckoutInput['paymentMethod'])}><SelectTrigger id="paymentMethod"><SelectValue /></SelectTrigger><SelectContent>{availablePaymentMethods.map((method) => <SelectItem key={method} value={method}>{method === 'paymob' ? t('paymob') : method === 'pay-on-delivery' ? t('payDelivery') : t('demoCard')}</SelectItem>)}</SelectContent></Select></div><label className="grid gap-1.5"><span className="text-sm font-bold text-foreground">{t('promoCode')}</span><div className="flex gap-2"><Input type="text" value={promo.code} onChange={(event) => promo.setCode(event.target.value)} aria-label={t('promoCode')} /><Button type="button" variant="outline" size="sm" onClick={promo.confirm}>{t('applyPromo')}</Button></div>{promo.state === 'valid' ? <small className="text-sm text-success">{t('promoApplied')} — {formatMoney(promo.discountMinor ?? 0, locale)}</small> : promo.error ? <small className="text-sm text-destructive">{promo.error === 'below_minimum' ? t('promoBelowMinimum') : promo.error === 'not_found' ? t('promoNotFound') : t('promoInvalid')}</small> : null}</label><label className="grid gap-1.5"><span className="text-sm font-bold text-foreground">{t('giftCardCode')}</span><Input type="text" autoComplete="off" value={input.giftCardCode ?? ''} onChange={(event) => update('giftCardCode', event.target.value)} aria-label={t('giftCardCode')} /></label><label className="flex items-center gap-3 rounded-2xl border border-border bg-card p-4 has-[:focus-visible]:ring-2 has-[:focus-visible]:ring-ring"><input type="checkbox" checked={simulateFailure} onChange={(event) => setSimulateFailure(event.target.checked)} className="accent-primary" /><span>{t('simulateFailure')}</span></label><Button type="submit" disabled={submitting}>{submitting ? t('processing') : t('placeOrder')} · {formatMoney(Math.max(0, liveTotal - promoDiscount), locale)} <span aria-hidden="true">↗</span></Button><p className="text-xs text-muted-foreground">{t('demoDisclosure')}</p></section>
  </form>;
}
