...
export function WishlistHeart({ slug, className = '' }: { slug: string; className?: string }) {
  ...
  return (
    <button
      ...
      className={`grid h-11 w-11 place-items-center rounded-full border bg-background/80 shadow-sm transition-colors hover:border-primary ${saved ? 'text-destructive' : 'text-muted-foreground'} ${className}`}
    >
