...
export function AuthorByline({ author, locale, city }: { author: Author | null; locale: Locale; city: string }) {
  ...
  return <Link className="mt-8 flex items-start gap-4 border-t border-border pt-6" href={`/${locale}/${city}/blog/authors/${author.slug}`}>
    {author.avatarUrl ? <Image src={author.avatarUrl} alt="" width={48} height={48} loading="lazy" className="h-12 w-12 rounded-full object-cover" /> : <span aria-hidden="true" className="grid h-12 w-12 place-items-center rounded-full bg-accent font-display text-lg text-primary">{name.slice(0, 1)}</span>}
    <div>
